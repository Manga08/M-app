...
export function DashboardPage() {
  ...
  return <div className="min-w-0" data-dashboard>
    <section className="min-w-0 border-b border-border/75 pb-8 lg:pb-10" data-dashboard-hero>
      ...
      <div className="mt-8 grid min-w-0 grid-cols-2 gap-y-6 sm:mt-10 md:grid-cols-[minmax(0,1.5fr)_1fr_1fr] md:gap-0">
        <div className="col-span-2 min-w-0 md:col-span-1 md:pr-7 lg:pr-8"><p className="text-xs text-muted-foreground">Disponible este mes</p><p className="mt-1 max-w-full overflow-hidden text-ellipsis text-[clamp(2.15rem,10vw,4.25rem)] font-medium leading-none tracking-[-0.06em] tabular-nums" data-dashboard-balance>{money.format(available)}</p><div className="mt-5 flex min-w-0 items-center gap-3 text-xs text-muted-foreground"><span className="h-1.5 w-24 shrink-0 overflow-hidden rounded-full bg-muted"><span className="block h-full rounded-full bg-primary" style={{ width: `${Math.max(0, Math.min(100, Math.round((available / Math.max(totals.income, 1)) * 100)))}%` }} /></span><span className="truncate">{Math.round((available / Math.max(totals.income, 1)) * 100)}% del ingreso disponible</span></div></div>
...
function Metric({ label, value, note, positive = false, mobileDivider = false }: { label: string; value: string; note: string; positive?: boolean; mobileDivider?: boolean }) { return <div className={cn("min-w-0 border-border md:border-l md:px-7", mobileDivider && "border-l pl-4 min-[360px]:pl-5 md:pl-7")}><p className="text-xs text-muted-foreground">{label}</p><p className="mt-2 truncate text-[clamp(1.05rem,5.6vw,1.5rem)] font-medium tracking-[-0.04em] tabular-nums">{value}</p><p className={cn("mt-1 truncate text-[11px] text-muted-foreground min-[360px]:text-xs", positive && "text-emerald-300")}>{note}</p></div>; }
