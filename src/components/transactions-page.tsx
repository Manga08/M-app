...
export function TransactionsPage({ embedded = false }: { embedded?: boolean }) {
  ...
  return <>
    ...
    <section aria-label="Historial de movimientos">
      ...
      <div className={cn("divide-y transition-opacity duration-150 motion-reduce:transition-none", loading && "opacity-45")}>{visibleRows.map((transaction) => {
...
function TransactionRowView({ transaction, category, targetName, icon, accountName, destinationName, money, income, onDelete }: { transaction: Transaction; category: string; targetName?: string; icon: string; accountName?: string; destinationName?: string; money: Intl.NumberFormat; income: boolean; onDelete: (id: string) => void }) {
  ...
  return <div role="listitem" data-transaction-id={transaction.id} className="grid min-h-[76px] grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2 py-3 transition-colors duration-150 hover:bg-secondary/25 xl:grid-cols-[110px_minmax(160px,1.4fr)_minmax(120px,1fr)_minmax(130px,1fr)_120px_44px] xl:gap-4">
