...
export function AccountsPage() {
  ...
  return <>
    ...
    <section className="pt-8"><div className="mb-5"><h2 className="text-xl font-medium tracking-tight">Todas las cuentas</h2><p className="mt-1 text-sm text-muted-foreground">Saldo calculado con tus movimientos.</p></div><div>{balances.map(({ account, balance }) => { const Icon = typeIcon[account.type]; return <button key={account.id} type="button" className="grid min-h-[82px] w-full grid-cols-[auto_minmax(0,1fr)_auto_auto] items-center gap-4 border-b py-3 text-left hover:bg-foreground/[0.025]"><span className="grid size-11 place-items-center rounded-2xl" style={{ backgroundColor: `${account.color}1f`, color: account.color }}><Icon className="size-5" /></span><span><span className="block text-sm font-medium">{account.name}</span><span className="block text-xs text-muted-foreground">{typeLabel[account.type]}</span></span><span className={cn("text-right text-base font-medium tabular-nums", balance < 0 && "text-destructive")}>{money.format(balance)}</span><ChevronRight className="size-4 text-muted-foreground" /></button>; })}</div></section>
    <AnimatePresence>{open && <motion.div className="fixed inset-0 z-50 grid place-items-end bg-black/55 p-0 backdrop-blur-sm sm:place-items-center sm:p-5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><motion.div role="dialog" aria-modal="true" aria-labelledby="new-account-title" className="w-full rounded-t-[1.75rem] border bg-popover p-6 sm:max-w-md sm:rounded-3xl" initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 40, opacity: 0 }}><div className="mb-6 flex items-center justify-between"><div><p className="text-xs uppercase tracking-[.14em] text-primary">Organiza tu dinero</p><h2 id="new-account-title" className="mt-1 text-2xl font-medium">Nueva cuenta</h2></div><Button variant="ghost" size="icon" onClick={() => setOpen(false)} aria-label="Cerrar"><X className="size-5" /></Button></div><form className="space-y-5" onSubmit={submit}><div><Label htmlFor="account-name">Nombre</Label><Input id="account-name" name="name" required className="mt-2 h-12" placeholder="Ej. Davivienda" /></div><div><Label htmlFor="account-type">Tipo</Label><select id="account-type" name="type" className="mt-2 h-12 w-full rounded-xl border bg-background px-3 text-sm"><option value="checking">Cuenta corriente</option><option value="savings">Ahorros</option><option value="cash">Efectivo</option><option value="credit">Crédito</option><option value="investment">Inversión</option></select></div><div><Label htmlFor="initial-balance">Saldo inicial</Label><Input id="initial-balance" name="initialBalance" type="number" className="mt-2 h-12" defaultValue="0" /></div><Button type="submit" className="h-12 w-full rounded-full">Crear cuenta</Button></form></motion.div></motion.div>}</AnimatePresence>
  </>;
}
