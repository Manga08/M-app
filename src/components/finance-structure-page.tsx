...
function StructureEditor({ groups, finance, embedded }: { groups: GroupAllocation[]; finance: ReturnType<typeof useFinance>; embedded: boolean }) {
  ...
  async function archiveGroup(group: GroupAllocation, destination?: string, archiveCategories = false) {
    ...
    try {
      ...
      announceMutation(result, destination ? `“${group.name}” se unió con el grupo elegido.` : `“${group.name}” se archivó sin borrar tu historial.`);
      return true;
    } catch (error) {
      announceMutationError(error, "No pudimos archivar el grupo.");
      return false;
    }
  }
  ...
  return <>
    {!embedded ? <PageHeader
      ...
      title="Plan"
      description="Diseña tus grupos principales, decide cuáles participan del reparto y organiza las subcategorías que usarás al registrar movimientos."
...
function GroupDialog({ open, group, nextOrder, onOpenChange, onSave }: { open: boolean; group?: GroupAllocation; nextOrder: number; onOpenChange: (open: boolean) => void; onSave: (group: FinanceGroupInput) => Promise<void> }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    catch (error) { announceMutationError(error, "No pudimos guardar el grupo principal."); }
