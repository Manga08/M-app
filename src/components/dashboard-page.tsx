...
import { groupBudgetSummary, monthTotals } from "@/lib/finance/calculations";
...
export function DashboardPage() {
  ...
  const groups = groupBudgetSummary(categories, budgets, transactions);
  ...
  return <>
    <section className="relative overflow-hidden border-b pb-8 lg:pb-10">
      ...
      <div className="relative flex flex-col justify-between gap-8 xl:flex-row xl:items-end">
        <div><p className="mb-2 text-sm text-muted-foreground">Hola, Andrés <span aria-hidden="true">👋</span></p><h1 className="text-balance text-[clamp(1.9rem,4vw,3.3rem)] font-medium leading-[1.04] tracking-[-0.055em]">Tu dinero, en calma.</h1><p className="mt-3 max-w-xl text-sm leading-6 text-muted-foreground">Vas al día con tu plan. Te quedan 14 días para cerrar agosto y aún tienes margen para disfrutar.</p></div>
      ...
      <div className="relative mt-9 grid gap-7 md:grid-cols-[minmax(0,1.5fr)_1fr_1fr] md:gap-0">
        <div className="md:pr-8"><p className="text-xs text-muted-foreground">Disponible este mes</p><p className="mt-1 text-[clamp(2.35rem,5vw,4.5rem)] font-medium leading-none tracking-[-0.06em] tabular-nums">{money.format(available)}</p><div className="mt-5 flex items-center gap-3 text-xs text-muted-foreground"><span className="h-1.5 w-24 overflow-hidden rounded-full bg-muted"><span className="block h-full rounded-full bg-primary" style={{ width: `${Math.max(0, Math.min(100, Math.round((available / Math.max(totals.income, 1)) * 100)))}%` }} /></span>{Math.round((available / Math.max(totals.income, 1)) * 100)}% del ingreso disponible</div></div>
    ...
    <section className="grid gap-10 py-8 xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,0.65fr)] xl:gap-14">
      <div>
        ...
        <div className="space-y-1">{groups.map((group) => <CategoryRow key={group.group} {...group} />)}</div>
      </div>
      <div className="border-t pt-8 xl:border-l xl:border-t-0 xl:pl-10 xl:pt-0">
        <div className="mb-5 flex items-center justify-between"><div><h2 className="text-lg font-medium tracking-tight">Pulso de agosto</h2><p className="mt-1 text-sm text-muted-foreground">Lo importante, sin ruido</p></div><CircleDollarSign className="size-5 text-primary" /></div>
        <div className="space-y-5"><Insight value="16%" label="más ahorrado que en julio" tone="text-emerald-300" /><Insight value={money.format(totals.expense / 17)} label="promedio diario de salida" /><Insight value={`${groups.filter((group) => group.percent >= 85).length} alertas`} label="categorías cerca del límite" tone="text-amber-300" /></div>
      </div>
    </section>
    ...
    <section className="border-t pt-8">
      ...
      <div>{recent.map((transaction) => <TransactionRow key={transaction.id} transaction={transaction} category={categories.find((category) => category.id === transaction.categoryId)?.name} />)}</div>
    </section>
  </>;
}
...
function CategoryRow({ group, budget, spent, percent }: ReturnType<typeof groupBudgetSummary>[number]) {
...
function TransactionRow({ transaction, category }: { transaction: Transaction; category?: string }) {
