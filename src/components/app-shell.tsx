...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const pageName = appNav.find((item) => isActivePath(pathname, item.href))?.label ?? (pathname.startsWith("/perfil") ? "Perfil" : "Tu espacio financiero");
  ...
  return <div className="min-h-screen bg-background noise">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[236px] border-r bg-background/90 px-4 py-5 backdrop-blur-xl lg:flex lg:flex-col">
      ...
      <nav className="mt-7 space-y-1" aria-label="Navegación principal">{appNav.map(({ label, href, icon: Icon }) => { const active = isActivePath(pathname, href); return <Link key={href} href={href} className={cn("group relative flex h-11 items-center gap-3 rounded-xl px-3 text-sm text-muted-foreground transition-[color,background-color,transform] hover:bg-secondary hover:text-foreground active:scale-[.985]", active && "bg-secondary text-foreground")}><span className={cn("grid size-8 place-items-center rounded-lg transition-colors", active && "bg-primary/12 text-primary")}><Icon className="size-[18px]" strokeWidth={1.8} /></span>{label}{active ? <motion.span layoutId="desktop-active-dot" className="ml-auto size-1.5 rounded-full bg-primary" /> : null}</Link>; })}</nav>
    ...
    <div className="min-h-screen lg:ml-[236px]">
      <header className="sticky top-0 z-20 flex h-[68px] items-center justify-between border-b bg-background/86 px-4 backdrop-blur-xl sm:px-5 md:px-8 lg:h-[76px] lg:border-b-0 lg:px-12">
      ...
      <AnimatePresence mode="wait" initial={false}>
        <motion.main key={pathname} initial={reduceMotion ? false : { opacity: 0, y: 7, filter: "blur(2px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} exit={reduceMotion ? undefined : { opacity: 0, y: -4, filter: "blur(1px)" }} transition={{ duration: 0.2, ease: [0.22, 1, 0.36, 1] }} className="mx-auto max-w-[1500px] px-4 pb-40 pt-5 sm:px-5 md:px-8 lg:px-12 lg:pb-10 lg:pt-5">{children}</motion.main>
      </AnimatePresence>
    </div>
    ...
    <nav className="fixed inset-x-0 bottom-0 z-30 border-t bg-background/94 px-2 pb-[max(0.55rem,env(safe-area-inset-bottom))] pt-1.5 backdrop-blur-xl lg:hidden" aria-label="Navegación móvil">
      <div className="mx-auto grid max-w-xl grid-cols-5 gap-1">{mobilePrimaryNav.map(({ label, href, icon: Icon }) => { const active = isActivePath(pathname, href); return <MobileNavLink key={href} active={active} href={href} label={label} icon={Icon} />; })}<button type="button" onClick={() => setMoreOpen(true)} className={cn("relative flex min-h-[54px] min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-medium text-muted-foreground transition-transform active:scale-95", moreActive && "text-primary")} aria-label="Abrir más opciones" aria-current={moreActive ? "page" : undefined}>{moreActive ? <motion.span layoutId="mobile-active-pill" className="absolute inset-x-1 top-0 h-10 rounded-xl bg-primary/10" transition={{ type: "spring", stiffness: 450, damping: 36 }} /> : null}<Ellipsis className="relative size-[19px]" /><span className="relative truncate">Más</span></button></div>
    </nav>

    <MobileMoreSheet open={moreOpen} onOpenChange={setMoreOpen} pathname={pathname} profile={profile} online={online} pendingCount={pendingCount} />
...
function MobileNavLink({ active, href, label, icon: Icon }: { active: boolean; href: string; label: string; icon: typeof LayoutDashboard }) {
  return <Link href={href} aria-current={active ? "page" : undefined} className={cn("relative flex min-h-[54px] min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-medium text-muted-foreground transition-transform active:scale-95", active && "text-primary")}>{active ? <motion.span layoutId="mobile-active-pill" className="absolute inset-x-1 top-0 h-10 rounded-xl bg-primary/10" transition={{ type: "spring", stiffness: 450, damping: 36 }} /> : null}<motion.span className="relative" animate={active ? { y: -1 } : { y: 0 }}><Icon className="size-[19px]" strokeWidth={active ? 2.2 : 1.8} /></motion.span><span className="relative w-full truncate text-center">{label}</span></Link>;
}

function MobileMoreSheet({ open, onOpenChange, pathname, profile, online, pendingCount }: { open: boolean; onOpenChange: (open: boolean) => void; pathname: string; profile: ReturnType<typeof useFinance>["profile"]; online: boolean; pendingCount: number }) {
