...
import { defaultReportQuery, normalizeReportQuery, parseReportQuery, reportDateMatchesQuery, reportPeriodLabel, reportQueryKey, reportRangeForPreset, serializeReportQuery } from "@/lib/finance/report-query";
...
import { financialTargetProgress, liabilityBackedTargetProgress } from "@/lib/finance/financial-targets";
...
export function ReportsPage() {
  ...
  async function exportXlsx() {
    ...
    try {
      ...
      const blob = await createReportWorkbook({ report: exportReport, query, transactions, accounts, accountEntities: finance.accountEntities, categories, profile, financialTargets: finance.financialTargets, financialTargetEntries: finance.financialTargetEntries, financialTargetDebts: finance.financialTargetDebts, creditCards: finance.creditCards, creditCardStatements: finance.creditCardStatements, creditCardPurchasePlans: finance.creditCardPurchasePlans, creditCardInstallments: finance.creditCardInstallments, liabilityOverview: finance.liabilityOverview });
  ...
  return <>
    ...
    {!offlineGuard && report ? <div className={cn("min-w-0 transition-opacity duration-[var(--motion-duration-state)] ease-[var(--motion-ease-out)] motion-reduce:duration-[var(--motion-duration-reduced)]", refreshing && "pointer-events-none opacity-65")} aria-busy={refreshing}>
      ...
      <CreditCardReport finance={finance} report={report} query={query} />
...
function CreditCardReport({ finance, report, query }: { finance: ReturnType<typeof useFinance>; report: DetailedFinanceReport; query: ReportQuery }) {
  const accountById = new Map(report.accounts.map((account) => [account.id, account]));
  const cards = finance.creditCards.flatMap((card) => {
    const account = accountById.get(card.accountId);
    if (!account) return [];
    const statements = finance.creditCardStatements.filter((statement) => statement.accountId === card.accountId && reportDateMatchesQuery(statement.cutoffOn, query) && creditCardStatementIsReconciled(statement));
    return [{ card, account, interest: statements.reduce((sum, statement) => sum + statement.interest, 0), fees: statements.reduce((sum, statement) => sum + statement.fees, 0) }];
  });
  if (!cards.length) return null;
  return <section className="border-b py-9" aria-labelledby="report-credit-cards-title"><div className="flex items-end justify-between gap-4"><SectionHeading eyebrow="Crédito" title="Tarjetas en el periodo" description="Compras y pagos usan la moneda nativa exacta. Un extracto solo se considera definitivo cuando fue conciliado." /><Button asChild variant="outline" className="mb-5 shrink-0 rounded-full"><Link href="/cuentas?vista=tarjetas">Gestionar</Link></Button></div><div className="border-y">{cards.map(({ card, account, interest, fees }) => { const nativeMoney = currencyFormatter(account.currencyCode); const debt = Math.max(0, -account.nativeClosingBalance); const utilization = card.creditLimit > 0 ? debt / card.creditLimit * 100 : 0; return <Link key={account.id} href={`/cuentas/tarjetas/${account.id}`} className="grid min-h-24 gap-4 border-b py-4 last:border-b-0 sm:grid-cols-[minmax(220px,1fr)_minmax(180px,.8fr)_minmax(180px,.7fr)] sm:items-center"><span className="flex min-w-0 items-center gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-xl" style={{ backgroundColor: `${account.color}18`, color: account.color }}><FinanceIcon name={account.icon || "credit-card"} className="size-5" /></span><span className="min-w-0"><span className="block truncate text-sm font-medium">{account.entityName ? `${account.entityName} · ` : ""}{account.name}</span><span className="mt-1 block text-xs text-muted-foreground">{card.network.toUpperCase()}{card.lastFour ? ` · •••• ${card.lastFour}` : ""} · {account.currencyCode}</span></span></span><span className="min-w-0"><span className="flex items-baseline justify-between gap-3 text-xs"><span className="text-muted-foreground">Uso del cupo</span><span className="font-medium tabular-nums">{Math.round(utilization)}%</span></span><Progress className="mt-2 h-1.5" value={Math.min(100, utilization)} label={`Uso del cupo de ${account.name}`} valueText={`${Math.round(utilization)}%`} indicatorClassName={utilization >= 90 ? "bg-destructive" : utilization >= 70 ? "bg-warning" : undefined} /><span className="mt-2 block text-[11px] text-muted-foreground">Compras {nativeMoney.format(account.nativeExpense)} · pagos {nativeMoney.format(account.nativeTransferIn)}</span></span><span className="text-right"><span className="block text-sm font-medium tabular-nums">{nativeMoney.format(debt)}</span><span className="mt-1 block text-[11px] text-muted-foreground">deuda al cierre</span>{interest || fees ? <span className="mt-1 block text-[11px] text-warning">{nativeMoney.format(interest + fees)} conciliados en interés y cargos</span> : null}</span></Link>; })}</div></section>;
}
...
function TargetReport({ finance, money }: { finance: ReturnType<typeof useFinance>; money: Intl.NumberFormat }) {
  ...
  return <section className="border-b py-9"><div className="flex items-end justify-between gap-4"><SectionHeading eyebrow="Rumbo financiero" title="Metas y deudas actuales" description="Estado actual del recorrido. El periodo elegido sigue controlando las gráficas y movimientos; esta sección muestra el saldo vivo de cada objetivo." /><Button asChild variant="outline" className="mb-5 shrink-0 rounded-full"><Link href="/metas">Gestionar</Link></Button></div>{targets.length ? <div className="grid gap-x-10 gap-y-2 md:grid-cols-2">{targets.map((target) => {
    const liability = target.kind === "debt" ? finance.liabilityOverview.items.find((item) => item.liability.legacyTargetId === target.id) : undefined;
    const progress = liability ? liabilityBackedTargetProgress(liability.originalPrincipal ?? target.targetAmount, liability.nativeDebt) : financialTargetProgress(target, finance.financialTargetEntries, finance.transactions);
    const amountMoney = liability ? currencyFormatter(liability.currencyCode) : money;
    const remaining = liability?.nativeDebt ?? progress.remaining;
    const next = liability?.nextObligation;
    const dataState = liability?.reportingApproximate ? "estimated" : liability?.nextObligation && liability.nextObligation.status !== "projected" ? "confirmed" : "calculated";
    return <Link key={target.id} href={`/metas?meta=${encodeURIComponent(target.id)}`} className="grid min-h-20 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 py-4"><span className="grid size-10 place-items-center rounded-xl" style={{ color: target.color, backgroundColor: `${target.color}16` }}><FinanceIcon name={target.icon} className="size-[18px]" /></span><span className="min-w-0"><span className="flex min-w-0 items-center gap-2"><span className="truncate text-sm font-medium">{target.title}</span>{liability ? <FinanceDataStateBadge state={dataState} /> : null}</span><span className="mt-2 flex items-center gap-2"><Progress className="max-w-44" indicatorClassName="bg-[var(--target-color)]" style={{ "--target-color": target.color } as React.CSSProperties} value={progress.rawProgress} max={liability?.originalPrincipal ?? target.targetAmount} label={`Avance de ${target.title}`} valueText={`${Math.round(progress.percent)}%`} /><span className="text-[11px] text-muted-foreground">{Math.round(progress.percent)}%</span></span></span><span className="text-right"><span className="block text-sm font-medium tabular-nums">{amountMoney.format(remaining)}</span><span className="block text-[11px] text-muted-foreground">{next ? `paga ${shortDate(next.dueOn)}` : target.mode === "pay_down" ? "por pagar" : "pendiente"}</span>{next ? <span className="block text-[11px] tabular-nums text-warning">{amountMoney.format(next.totalDue)}</span> : null}</span></Link>;
  })}</div> : <EmptyInline text="Todavía no has creado metas ni deudas." />}</section>;
}
