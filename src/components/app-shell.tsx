...
import { CalendarDays, CloudOff, FolderCog, LayoutDashboard, LineChart, Menu, Plus, ReceiptText, Settings2, Target, UserRound, WalletCards } from "lucide-react";
...
const desktopNav = [
  ...appNav.slice(0, 3),
  { label: "Estructura", shortLabel: "Estructura", href: "/estructura", icon: FolderCog },
  ...appNav.slice(3),
] as const;
...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const pageName = desktopNav.find((item) => item.href === "/" ? pathname === "/" : pathname.startsWith(item.href))?.label ?? (pathname.startsWith("/perfil") ? "Perfil" : "Tu espacio financiero");
  ...
  return <div className="min-h-screen bg-background noise">
    ...
    {!pathname.startsWith("/estructura") ? <Button onClick={() => { setEditingTransactionId(undefined); setQuickAddOpen(true); }} className="fixed bottom-24 right-4 z-20 size-14 rounded-full p-0 shadow-[0_16px_40px_-14px_var(--primary)] sm:right-5 lg:hidden" aria-label="Registrar movimiento"><Plus className="size-6" /></Button> : null}
    <nav className="fixed inset-x-0 bottom-0 z-30 border-t bg-background/94 px-1 pb-[max(0.55rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl lg:hidden" aria-label="Navegación móvil"><div className="mx-auto grid max-w-xl grid-cols-6">{appNav.map(({ shortLabel, href, icon: Icon }) => { const active = href === "/" ? pathname === "/" : pathname.startsWith(href) || (href === "/presupuestos" && pathname.startsWith("/estructura")); return <Link key={href} href={href} className={cn("flex min-h-12 min-w-0 flex-col items-center justify-center gap-1 rounded-lg px-0.5 text-[9px] text-muted-foreground", active && "text-primary")}><Icon className="size-[18px]" /><span className="w-full truncate text-center">{shortLabel}</span></Link>; })}</div></nav>
