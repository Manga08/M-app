...
import { CalendarClock, ChevronDown, CreditCard, LoaderCircle, Pencil, Plus, ShieldCheck, WalletCards } from "lucide-react";
...
import type { Account, CreditCardInput, CreditCardNetwork, CreditCardProfile, CreditCardStatement } from "@/lib/finance/types";
...
export function CreditCardsPage() {
  const { accountEntities, accounts, creditCards, creditCardStatements, transactions, snapshot, profile } = useFinance();
  ...
  const activeCards = useMemo(() => creditCards.flatMap((card) => {
    ...
    const statement = creditCardStatements
      .filter((item) => item.accountId === card.accountId)
      .sort((a, b) => b.cutoffOn.localeCompare(a.cutoffOn))[0];
    return [{ card, account, debt, available: creditCardAvailable(card.creditLimit, debt), utilization: creditCardUtilization(card.creditLimit, debt), cycle, statement }];
  }).sort((a, b) => creditCardUrgency(a.cycle, a.debt) - creditCardUrgency(b.cycle, b.debt) || b.utilization - a.utilization), [accounts, creditCards, creditCardStatements, snapshot, transactions]);
  ...
  const dueSoon = activeCards.filter((item) => item.debt > 0 && item.cycle.daysUntilUpcomingDue <= 7).length;
    ...
    <section className="pt-7 lg:pt-10" aria-labelledby="credit-cards-title">
      <div className="mb-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">La deuda se calcula desde tus movimientos. Un extracto conciliado confirma cuánto y cuándo pagar.</p>
        </div>
...
function CardRow({ account, card, debt, available, utilization, cycle, statement, onEdit }: { account: Account; card: CreditCardProfile; debt: number; available: number; utilization: number; cycle: ReturnType<typeof creditCardCycle>; statement?: CreditCardStatement; onEdit: () => void }) {
  ...
  const urgency = debt > 0 && cycle.daysUntilUpcomingDue <= 7;
  const statementState = creditCardStatementIsReconciled(statement) ? "confirmed" : statement ? "manual" : "estimated";
  return <article role="listitem" className="group relative border-b last:border-b-0">
    <Link href={`/cuentas/tarjetas/${account.id}`} className="grid min-h-32 gap-5 px-1 py-5 outline-none transition-colors duration-[var(--motion-duration-state)] ease-[var(--motion-ease-out)] hover:bg-secondary/20 focus-visible:bg-secondary/25 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring sm:grid-cols-[minmax(220px,1fr)_minmax(200px,.85fr)_minmax(180px,.75fr)] sm:items-center sm:px-3">
      ...
      <div className="flex items-end justify-between gap-4 sm:block sm:text-right">
        <div><p className="text-lg font-medium tabular-nums">{money.format(debt)}</p><p className="mt-1 text-xs text-muted-foreground">deuda actual · calculada</p></div>
        <div className={cn("text-xs", urgency ? "text-destructive" : "text-muted-foreground")}><p>{statement ? `Extracto ${money.format(statement.totalDue)}` : cycle.daysUntilCutoff === 0 ? "Corta hoy" : `Corte en ${cycle.daysUntilCutoff} días`}</p><p className="mt-1">Pago {formatDate(statement?.dueOn ?? cycle.upcomingDueOn)}</p><FinanceDataStateBadge state={statementState} className="mt-2" /></div>
      </div>
    </Link>
...
function CreditCardDialog({ open, account, card, entities, onOpenChange, onSaved }: { open: boolean; account?: Account; card?: CreditCardProfile; entities: Array<{ id: string; name: string }>; onOpenChange: (open: boolean) => void; onSaved: () => void }) {
  ...
  return <Dialog open={open} onOpenChange={(next) => !saving && onOpenChange(next)}><FormDialogContent variant="flow" showCloseButton={!saving}><form className="flex min-h-0 flex-1 flex-col" onSubmit={submit}><FormDialogBody>
    <DialogHeader className="mb-7 pr-8"><p className="text-xs uppercase tracking-[.14em] text-primary">Producto de crédito</p><DialogTitle className="text-2xl">{editing ? "Editar tarjeta" : "Nueva tarjeta"}</DialogTitle><DialogDescription>Primero identifica la tarjeta y su cupo. El ciclo, las tasas y los cargos quedan como datos manuales hasta que concilies un extracto.</DialogDescription></DialogHeader>
    <div className="grid gap-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(300px,.8fr)] lg:gap-12">
      <div className="space-y-5">
        <div className="flex items-start justify-between gap-4"><div><p className="font-medium">Identidad y cupo</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Lo necesario para reconocerla y calcular cuánto crédito queda.</p></div><FinanceDataStateBadge state="manual" /></div>
      ...
      <div className="space-y-5">
        <div className="flex items-start justify-between gap-4"><div><p className="font-medium">Ciclo de facturación</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Sirve para anticipar corte y vencimiento. Si el mes tiene menos días, se usa su último día válido.</p></div><FinanceDataStateBadge state="estimated" /></div>
        ...
        <div className="rounded-2xl bg-secondary/30 p-4"><div className="flex items-start gap-3"><CalendarClock className="mt-0.5 size-5 shrink-0 text-primary" aria-hidden="true" /><div><p className="text-sm font-medium">Una compra se registra una sola vez</p><p className="mt-1 text-xs leading-5 text-muted-foreground">A una cuota queda en el ciclo como gasto completo. Si eliges varias cuotas, Moneva muestra el calendario sin volver a sumar el gasto.</p></div></div></div>
        <details className="group rounded-2xl bg-secondary/25 px-4 py-3.5">
          <summary className="coarse-target flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-medium"><span><span className="block">Tasas y cargos</span><span className="mt-0.5 block text-xs font-normal text-muted-foreground">Opcional · configuración avanzada</span></span><ChevronDown className="size-4 shrink-0 text-muted-foreground transition-transform duration-[var(--motion-duration-state)] ease-[var(--motion-ease-out)] group-open:rotate-180 motion-reduce:transition-none" aria-hidden="true" /></summary>
          <div className="mt-5 space-y-5"><div className="flex items-center justify-between gap-4"><p className="text-xs leading-5 text-muted-foreground">Copia los valores que informa el banco. Un extracto posterior puede confirmarlos.</p><FinanceDataStateBadge state="manual" /></div><Field label="Cuota de manejo anual" optional htmlFor="credit-card-annual-fee"><InputControl id="credit-card-annual-fee" inputMode="decimal" value={annualFee} onChange={(event) => setAnnualFee(formatMoneyInput(event.target.value, currencyCode))} leading={<span className="text-xs font-medium">{currencyCode === "USD" ? "US$" : "$"}</span>} /></Field><div className="grid gap-5 sm:grid-cols-2"><Field label="Tasa de compras E.A." optional htmlFor="credit-card-purchase-rate"><InputControl id="credit-card-purchase-rate" inputMode="decimal" value={purchaseRate} onChange={(event) => setPurchaseRate(cleanPercentage(event.target.value))} trailing={<span className="text-xs font-medium">%</span>} /></Field><Field label="Tasa de avances E.A." optional htmlFor="credit-card-advance-rate"><InputControl id="credit-card-advance-rate" inputMode="decimal" value={advanceRate} onChange={(event) => setAdvanceRate(cleanPercentage(event.target.value))} trailing={<span className="text-xs font-medium">%</span>} /></Field></div></div>
        </details>
...
const cleanPercentage = sanitizePercentageInput;
