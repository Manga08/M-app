...
import { nextPlannedOccurrence } from "@/lib/finance/recurrence";
...
export function ScheduledMovementsPage() {
  ...
  async function archiveRule(rule: RecurringRule) {
    if (busyId || !window.confirm(`¿Archivar “${rule.description}”? Los movimientos ya registrados se conservarán.`)) return;
    setBusyId(rule.id);
    try {
  ...
  return <div className="min-w-0">
    <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-[11px] font-medium uppercase tracking-[.14em] text-primary">Automatizaciones</p><h2 className="mt-2 text-2xl font-medium tracking-[-.035em]">Lo que se repite, una sola vez</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Configura suscripciones, salarios o transferencias. Moneva crea cada movimiento en la fecha elegida sin tocar saldos antes de tiempo.</p></div><Button className="h-11 rounded-full" onClick={() => window.dispatchEvent(new Event("moneva:quick-add"))}><Plus className="size-4" />Nueva programación</Button></div>
    ...
    <div className="mt-2 divide-y border-y">{visible.map((rule) => {
      ...
      return <article key={rule.id} className="grid min-h-[86px] grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 py-4 sm:grid-cols-[auto_minmax(0,1.3fr)_minmax(150px,.65fr)_minmax(130px,.55fr)_44px] sm:gap-4">
        ...
        <div className="min-w-0"><div className="flex min-w-0 items-center gap-2"><h3 className="truncate text-sm font-medium">{rule.description}</h3>{rule.status === "paused" ? <span className="rounded-full bg-warning/12 px-2 py-0.5 text-[10px] font-medium text-warning">Pausado</span> : null}</div><p className="mt-1 truncate text-xs text-muted-foreground">{scheduleText(rule)} · {category?.name ?? (rule.kind === "transfer" ? `${account?.name ?? "Cuenta"} → ${destination?.name ?? "Cuenta"}` : "Sin categoría")}</p></div>
...
function Summary({ label, value, helper, tone }: { label: string; value: string; helper: string; tone: "primary" | "positive" | "destructive" | "neutral" }) { return <div className="bg-background px-1 py-6 sm:px-5"><p className="text-xs text-muted-foreground">{label}</p><p className={cn("mt-2 truncate text-xl font-medium tabular-nums", tone === "primary" && "text-primary", tone === "positive" && "text-positive", tone === "destructive" && "text-destructive")}>{value}</p><p className="mt-1 truncate text-xs text-muted-foreground">{helper}</p></div>; }
...
function scheduleText(rule: RecurringRule) { const interval = rule.intervalCount > 1 ? `cada ${rule.intervalCount} ` : "cada "; if (rule.cadence === "weekly") return `${interval}${rule.intervalCount > 1 ? "semanas" : "semana"}`; if (rule.cadence === "yearly") return `${interval}${rule.intervalCount > 1 ? "años" : "año"}`; return rule.postingPolicy === "month_start" ? "el primer día de cada mes" : `el día ${rule.anchorDay ?? Number(rule.startsOn.slice(8, 10))} de cada mes`; }
