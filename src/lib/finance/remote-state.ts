...
type AccountRow = { id: string; name: string; account_type: Account["type"]; initial_balance: number | string; color: string; icon: string; archived: boolean; currency_code?: string; expected_annual_return?: number | string | null; version?: number | string };
...
export type TransactionRow = { id: string; kind: Transaction["kind"]; amount: number | string; account_id: string; category_id: string | null; transfer_group_id: string | null; recurring_occurrence_id: string | null; financial_target_id: string | null; financial_target_effect: Transaction["financialTargetEffect"] | null; description: string; merchant: string | null; note: string | null; icon: string | null; occurred_on: string; created_at: string; ledger_event_id?: string; native_currency_code?: string; base_currency_code?: string; base_amount?: number | string; exchange_rate?: number | string; exchange_rate_date?: string; exchange_rate_source?: Transaction["exchangeRateSource"]; version?: number | string };
export type RecurringRuleRow = { id: string; kind: RecurringRule["kind"]; amount: number | string; account_id: string; destination_account_id: string | null; category_id: string | null; financial_target_id: string | null; financial_target_effect: RecurringRule["financialTargetEffect"] | null; description: string; merchant: string | null; note: string | null; icon: string | null; cadence: RecurringRule["cadence"]; interval_count: number; starts_on: string; ends_on: string | null; anchor_day: number | null; weekday: number | null; posting_policy: RecurringRule["postingPolicy"]; timezone: string; auto_post: boolean; include_in_budget: boolean; include_in_income_target: boolean; status: RecurringRule["status"]; next_run_on: string | null; created_at: string; updated_at: string };
...
function snapshotFromRow(row: SnapshotRow): FinanceSnapshot {
  return { month: row.month, income: Number(row.income), expense: Number(row.expense), accountBalances: Object.fromEntries(Object.entries(row.accountBalances ?? {}).map(([id, value]) => [id, Number(value)])), categorySpending: Object.fromEntries(Object.entries(row.categorySpending ?? {}).map(([id, value]) => [id, Number(value)])) };
}
...
export async function loadRemoteFinanceState(client: FinanceSupabaseClient): Promise<FinanceState> {
  ...
  const results = await Promise.all([
    ...
    client.from("accounts").select("id,name,account_type,initial_balance,color,icon,archived,currency_code,expected_annual_return,version").eq("archived", false).order("created_at"),
