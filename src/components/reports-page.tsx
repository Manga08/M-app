...
export function ReportsPage() {
  ...
  return <>
    ...
    {!offlineGuard && report ? <div className={cn("min-w-0 transition-opacity duration-[var(--motion-duration-state)] ease-[var(--motion-ease-out)] motion-reduce:duration-[var(--motion-duration-reduced)]", refreshing && "pointer-events-none opacity-65")} aria-busy={refreshing}>
      ...
      <CreditCardReport finance={finance} report={report} />
...
function CreditCardReport({ finance, report }: { finance: ReturnType<typeof useFinance>; report: DetailedFinanceReport }) {
  ...
  const cards = finance.creditCards.flatMap((card) => {
    ...
    if (!account) return [];
    const statements = finance.creditCardStatements.filter((statement) => statement.accountId === card.accountId && statement.cutoffOn >= report.startDate && statement.cutoffOn <= report.endDate);
