...
export function MovementsPage() {
  ...
  return <>
    <PageHeader eyebrow={monthLabel(currentMonth)} title="Movimientos" description="Registra lo que pasó, automatiza lo que se repite y entiende tu mes en el calendario." action={<Button className="h-11 rounded-full px-5 max-sm:hidden" onClick={() => window.dispatchEvent(new Event("moneva:quick-add"))}><Plus className="size-4" />Nuevo movimiento</Button>} />
    <nav className="sticky top-[calc(68px+env(safe-area-inset-top))] z-10 -mx-4 mb-7 border-b bg-background/96 px-4 pb-3 backdrop-blur-md sm:static sm:mx-0 sm:mb-9 sm:border-0 sm:bg-transparent sm:px-0 sm:pb-0 sm:backdrop-blur-none" aria-label="Secciones de movimientos">
    ...
    <Activity mode={view === "history" ? "visible" : "hidden"}><section id="movement-panel-history" role="tabpanel" aria-labelledby="movement-tab-history"><TransactionsPage embedded /></section></Activity>
    <Activity mode={view === "scheduled" ? "visible" : "hidden"}><section id="movement-panel-scheduled" role="tabpanel" aria-labelledby="movement-tab-scheduled"><ScheduledMovementsPage /></section></Activity>
    <Activity mode={view === "calendar" ? "visible" : "hidden"}><section id="movement-panel-calendar" role="tabpanel" aria-labelledby="movement-tab-calendar"><MovementCalendar /></section></Activity>
  </>;
}
