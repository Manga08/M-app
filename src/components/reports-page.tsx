...
function CashflowChart({ report }: { report: FinanceReport }) {
  ...
  return <div className="relative h-[280px] border-b border-l"><div className="absolute inset-x-0 top-1/4 border-t border-dashed" /><div className="absolute inset-x-0 top-1/2 border-t border-dashed" /><div className="absolute inset-x-0 top-3/4 border-t border-dashed" /><span className="absolute -left-1 top-0 -translate-x-full text-[10px] text-muted-foreground">{compactMoney.format(maxValue)}</span><svg viewBox="0 0 100 100" className="absolute inset-0 size-full overflow-visible" preserveAspectRatio="none" role="img" aria-label="Gráfica de ingresos y gastos de los últimos doce meses"><polyline points={points("income")} fill="none" stroke="var(--primary)" strokeWidth="1.6" vectorEffect="non-scaling-stroke" /><polyline points={points("expense")} fill="none" stroke="#fb7185" strokeWidth="1.6" vectorEffect="non-scaling-stroke" /></svg><div className="absolute -bottom-7 inset-x-0 flex justify-between text-[10px] text-muted-foreground">{report.months.map((item, index) => <span key={item.month} className={index % 2 ? "hidden sm:inline" : undefined}>{formatMonth(item.month)}</span>)}</div></div>;
}
