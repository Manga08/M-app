-- Changing the currency of an unused account is safe, but its opening equity
-- must be revalued with the rate supplied by the user. Accounts with ledger
-- history remain immutable by design.
