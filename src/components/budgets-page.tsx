...
function BudgetEditor({ amount, currencyCode, onSave }: { amount: number; currencyCode: string; onSave: (amount: number) => Promise<void> | void }) {
  ...
  return <div className="grid w-full grid-cols-[44px_minmax(0,1fr)_44px_44px] items-center gap-2 sm:w-auto sm:grid-cols-[28px_104px_28px_28px] sm:gap-1"><Button type="button" variant="outline" size="icon-sm" aria-label="Reducir presupuesto" disabled={saving} onClick={() => setInputValue(formatMoneyInputValue(Math.max(0, value - 50000), currencyCode))}><Minus className="size-[18px] sm:size-3.5" /></Button><input aria-label="Monto presupuestado" className="h-11 min-w-0 rounded-xl border bg-secondary/20 px-3 text-right text-base tabular-nums outline-none transition-[border-color,box-shadow] focus:border-ring focus:ring-3 focus:ring-ring/30 sm:h-8 sm:rounded-lg sm:px-2 sm:text-sm" inputMode="decimal" value={inputValue} disabled={saving} onChange={(event) => setInputValue(formatMoneyInput(event.target.value, currencyCode))} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void save(); } }} /><Button type="button" variant="outline" size="icon-sm" aria-label="Aumentar presupuesto" disabled={saving} onClick={() => setInputValue(formatMoneyInputValue(value + 50000, currencyCode))}><Plus className="size-[18px] sm:size-3.5" /></Button><Button type="button" size="icon-sm" aria-label="Guardar presupuesto" aria-hidden={!changed} tabIndex={changed ? 0 : -1} className={cn("transition-[opacity,transform]", !changed && "invisible opacity-0")} disabled={!changed || saving} onClick={() => void save()}>{saving ? <LoaderCircle className="size-[18px] animate-spin sm:size-3.5" /> : <Check className="size-[18px] sm:size-3.5" />}</Button></div>;
}
