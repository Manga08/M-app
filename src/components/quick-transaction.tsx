...
export function QuickTransaction({ open, transactionId, recurringRuleId, initialFinancialTargetId, initialTiming, initialType, initialTargetEffect, initialOccurredOn, onOpenChange, onExitComplete }: { open: boolean; transactionId?: string; recurringRuleId?: string; initialFinancialTargetId?: string; initialTiming?: "recurring"; initialType?: TransactionInput["type"]; initialTargetEffect?: "advance" | "reverse"; initialOccurredOn?: string; onOpenChange: (open: boolean) => void; onExitComplete?: () => void }) {
  ...
  return <><Dialog open={open} onOpenChange={requestOpenChange}>
    <DialogContent showCloseButton={false} onExitComplete={onExitComplete} className="flex max-h-[94dvh] flex-col gap-0 overflow-hidden p-0 sm:max-w-5xl max-sm:inset-0 max-sm:h-dvh max-sm:max-h-none max-sm:max-w-none max-sm:translate-x-0 max-sm:translate-y-0 max-sm:rounded-none max-sm:p-0 max-sm:pb-0">
      <form onSubmit={submit} className="relative flex min-h-0 min-w-0 flex-1 lg:grid lg:grid-cols-[minmax(0,1.45fr)_minmax(300px,.55fr)]">
        <div data-quick-transaction-body className="safe-dialog-top mobile-scroll min-h-0 min-w-0 flex-1 overflow-y-auto px-4 pb-40 pt-5 min-[360px]:px-5 sm:px-8 sm:pb-8 sm:pt-7">
          ...
          ] as const).map(({ value, label, icon: Icon }) => <button key={value} type="button" aria-pressed={form.type === value} disabled={Boolean(transactionId) && lockedType !== value} onClick={() => changeType(value)} className={cn("flex min-h-14 items-center justify-center gap-2 rounded-xl px-1.5 text-xs font-medium text-muted-foreground transition-[color,background-color,box-shadow,transform] duration-[var(--motion-duration-menu)] ease-[var(--motion-ease-out)] active:scale-[var(--motion-press-scale)] motion-reduce:transition-none max-[359px]:flex-col max-[359px]:gap-1 max-[359px]:text-[11px] sm:text-sm", form.type === value && "bg-background text-primary shadow-sm ring-1 ring-foreground/6", transactionId && lockedType !== value && "cursor-not-allowed opacity-35")}><Icon className="size-4" />{label}</button>)}</div>
          ...
          <div className="mt-6 grid min-w-0 gap-5 sm:grid-cols-2">
            <FieldSelect label={form.type === "transfer" ? "Desde" : "Cuenta"} value={form.accountId} onChange={(value) => { const nextCurrency = accounts.find((item) => item.id === value)?.currencyCode ?? profile?.currencyCode ?? "COP"; setForm((current) => ({ ...current, accountId: value, amount: formatMoneyInputValue(parseMoneyInput(current.amount), nextCurrency) })); rateTouchedRef.current = false; rateSourceRef.current = undefined; setDestinationTouched(false); setError(null); }} icon={<CreditCard className="size-4" />} options={accounts.map((item) => ({ value: item.id, label: `${item.name} · ${item.currencyCode ?? "COP"}` }))} invalid={Boolean(error?.includes("cuenta") && !error?.includes("destino"))} describedBy={error ? "transaction-form-error" : undefined} />
            {form.type === "transfer" ? <FieldSelect label="Hacia" value={form.destinationAccountId} onChange={(value) => { setForm((current) => ({ ...current, destinationAccountId: value, destinationAmount: "" })); rateTouchedRef.current = false; rateSourceRef.current = undefined; setDestinationTouched(false); setError(null); }} icon={<Landmark className="size-4" />} options={accounts.map((item) => ({ value: item.id, label: `${item.name} · ${item.currencyCode ?? "COP"}` }))} invalid={Boolean(error?.includes("destino") || error?.includes("diferentes"))} describedBy={error ? "transaction-form-error" : undefined} /> : form.type === "income" ? <div><FieldSelect label="Tipo de ingreso" value={form.categoryId} onChange={(value) => { const next = categories.find((item) => item.id === value); setForm((current) => ({ ...current, categoryId: value, icon: iconTouched ? current.icon : next?.icon ?? "" })); setError(null); }} icon={<BadgeDollarSign className="size-4" />} options={incomeTypes.map((item) => ({ value: item.id, label: item.name }))} emptyLabel="Crea un tipo en Cuentas" invalid={Boolean(error?.includes("tipo de ingreso"))} describedBy={error ? "transaction-form-error" : undefined} />{!incomeTypes.length ? <Link href="/cuentas#tipos-de-ingreso" className="mt-2 inline-flex text-xs font-medium text-primary underline-offset-4 hover:underline">Crear un tipo de ingreso en Cuentas</Link> : null}</div> : <>
...
function formFromTransaction(selected: ReturnType<typeof useFinance>["transactions"][number], transferPair: ReturnType<typeof useFinance>["transactions"][number] | undefined, accounts: ReturnType<typeof useFinance>["accounts"], categories: ReturnType<typeof useFinance>["categories"], currencyCode = "COP"): FormState {
  ...
  return { timing: "now", type, amount: formatMoneyInputValue(outgoing?.amount ?? selected.amount, sourceCurrency), destinationAmount: incoming ? formatMoneyInputValue(incoming.amount, destinationCurrency) : "", feeAmount: "", exchangeRate: outgoing?.exchangeRate ? formatMoneyInputValue(outgoing.exchangeRate, "USD") : selected.exchangeRate ? formatMoneyInputValue(selected.exchangeRate, "USD") : "", referenceExchangeRate: outgoing?.referenceExchangeRate ?? selected.referenceExchangeRate, accountId: outgoing?.accountId ?? selected.accountId, destinationAccountId: incoming?.accountId ?? accounts.find((item) => item.id !== selected.accountId)?.id ?? "", groupKey: type === "expense" ? categories.find((item) => item.id === selected.categoryId)?.group ?? "" : "", categoryId: selected.categoryId ?? "", occurredOn: selected.occurredOn, merchant: selected.merchant ?? "", description: selected.description, note: selected.note ?? "", icon: selected.icon ?? "", cadence: "monthly", intervalCount: 1, firstAnchorDay: 15, secondAnchorDay: 31, postingPolicy: "scheduled_date", autoPost: true, includeInBudget: false, includeInIncomeTarget: false, endsOn: "", financialTargetId: targetSource?.financialTargetId ?? "", financialTargetEffect: targetSource?.financialTargetEffect ?? "advance" };
}
