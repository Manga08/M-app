...
import { ArrowDownLeft, ArrowRightLeft, ArrowUpRight, BadgeDollarSign, CalendarDays, CreditCard, Landmark, Sparkles, Tag } from "lucide-react";
...
export function QuickTransaction({ open, transactionId, onOpenChange }: { open: boolean; transactionId?: string; onOpenChange: (open: boolean) => void }) {
  ...
  const [form, setForm] = useState<FormState>(() => selected ? formFromTransaction(selected, transferPair, accounts, profile?.currencyCode) : emptyForm(accounts[0]?.id, accounts[1]?.id, categories.find((category) => category.kind === "expense" && !category.archived)?.id));
    ...
    <DialogContent showCloseButton={!saving} className="flex max-h-[94dvh] flex-col gap-0 overflow-hidden p-0 sm:max-w-5xl max-sm:inset-0 max-sm:h-dvh max-sm:max-h-none max-sm:max-w-none max-sm:translate-x-0 max-sm:translate-y-0 max-sm:rounded-none max-sm:p-0 max-sm:pb-0">
      <form onSubmit={submit} className="relative flex min-h-0 flex-1 lg:grid lg:grid-cols-[minmax(0,1.45fr)_minmax(300px,.55fr)]">
        <div className="mobile-scroll min-h-0 flex-1 overflow-y-auto px-4 pb-40 pt-5 min-[360px]:px-5 sm:px-8 sm:pb-8 sm:pt-7">
          ...
          <div className="mt-7"><Label htmlFor="transaction-amount">Monto</Label><InputControl id="transaction-amount" value={form.amount} onChange={(event) => setForm((current) => ({ ...current, amount: formatMoneyInput(event.target.value, profile?.currencyCode) }))} inputMode="decimal" required placeholder="0" leading={<span className="text-xl font-medium">$</span>} containerClassName="mt-2 h-[72px] rounded-[20px] bg-secondary/35" className="pr-4 text-3xl font-medium tracking-[-.04em] tabular-nums" /></div>
          ...
          <m.div key={form.type} initial={{ opacity: 0.68, y: 3 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.14, ease: [0.2, 0, 0, 1] }} className="mt-6 grid gap-5 sm:grid-cols-2">
            ...
            {form.type === "transfer" ? <FieldSelect label="Hacia" value={form.destinationAccountId} onChange={(value) => setForm({ ...form, destinationAccountId: value })} icon={<Landmark className="size-4" />} options={accounts.map((item) => ({ value: item.id, label: item.name }))} /> : <FieldSelect label={form.type === "income" ? "Tipo de ingreso" : "Categoría"} value={form.categoryId} onChange={(value) => { const next = categories.find((item) => item.id === value); setForm({ ...form, categoryId: value, icon: iconTouched ? form.icon : next?.icon ?? "" }); }} icon={form.type === "income" ? <BadgeDollarSign className="size-4" /> : <Tag className="size-4" />} options={availableCategories.map((item) => ({ value: item.id, label: item.name }))} emptyLabel={form.type === "income" ? "Crea un tipo en Cuentas" : "No hay categorías disponibles"} />}
...
function FieldSelect({ label, value, onChange, options, icon, emptyLabel = "Selecciona" }: { label: string; value: string; onChange: (value: string) => void; options: Array<{ value: string; label: string }>; icon: React.ReactNode; emptyLabel?: string }) {
  const id = `transaction-${label.toLowerCase().replaceAll(" ", "-")}`;
  return <div><Label htmlFor={id}>{label}</Label><SelectControl id={id} value={value} onChange={(event) => onChange(event.target.value)} required leading={icon} containerClassName="mt-2 [&_[data-slot=form-control-leading]]:text-primary"><option value="" disabled>{options.length ? "Selecciona" : emptyLabel}</option>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</SelectControl></div>;
}
...
function validate(input: TransactionInput) {
  ...
  if (input.type !== "transfer" && !input.categoryId) return input.type === "income" ? "Selecciona un tipo de ingreso." : "Selecciona una categoría.";
  return null;
}
