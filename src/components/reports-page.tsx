  ...
  return <>
    ...
    <section className="sticky top-[calc(68px+env(safe-area-inset-top))] z-20 -mx-4 border-y bg-background/96 px-4 py-3 backdrop-blur-md sm:static sm:mx-0 sm:rounded-[1.25rem] sm:border sm:bg-secondary/18 sm:p-2 sm:backdrop-blur-none" aria-label="Periodo y filtros del reporte">
      <div className="flex min-w-0 items-center gap-2 sm:flex-wrap">
        ...
        <Sheet open={filterOpen} onOpenChange={setFilterOpen}><SheetTrigger asChild><Button variant="outline" className="h-11 shrink-0 rounded-full px-4 sm:h-9"><SlidersHorizontal className="size-4" />Filtros{filterCount ? <span className="grid size-5 place-items-center rounded-full bg-primary text-[10px] text-primary-foreground">{filterCount}</span> : null}</Button></SheetTrigger><SheetContent side="right" className="mobile-scroll h-dvh w-full gap-0 overflow-y-auto overscroll-y-contain p-0 sm:max-w-md"><ReportFilters query={query} onApply={(next) => { applyQuery(next); setFilterOpen(false); }} /></SheetContent></Sheet>
