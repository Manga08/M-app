...
function TargetMetric({ label, value }: { label: string; value: string }) { return <span className="min-w-0 bg-background px-4 py-4"><span className="block text-[11px] text-muted-foreground">{label}</span><span className="mt-1 block truncate text-sm font-medium tabular-nums">{value}</span></span>; }
...
function TargetDialog({ open, target, onOpenChange }: { open: boolean; target?: FinancialTarget; onOpenChange: (open: boolean) => void }) {
  ...
  return <Dialog open={open} onOpenChange={(next) => !saving && onOpenChange(next)}><DialogContent showCloseButton={!saving} className="max-sm:inset-0 max-sm:h-dvh max-sm:max-h-none max-sm:max-w-none max-sm:translate-x-0 max-sm:translate-y-0 max-sm:rounded-none sm:max-w-2xl"><form onSubmit={submit}><DialogHeader><DialogTitle>{target ? "Editar recorrido" : "Nueva meta o deuda"}</DialogTitle><DialogDescription>Define el destino y cómo quieres medirlo. El progreso siempre conservará su origen.</DialogDescription></DialogHeader><div className="grid gap-5 py-5 sm:grid-cols-2">
    ...
    <div><Label>Icono</Label><div className="mt-2"><FinanceIconPicker value={icon} onValueChange={setIcon} /></div></div>
    <div className="sm:col-span-2"><Label>Color</Label><div className="mt-2 flex flex-wrap gap-3">{targetColors.map((value) => <button key={value} type="button" onClick={() => setColor(value)} aria-label={`Usar color ${value}`} aria-pressed={color === value} className="size-11 rounded-full transition-transform duration-150 active:scale-95 motion-reduce:transition-none" style={{ backgroundColor: value, outline: color === value ? `2px solid ${value}` : undefined, outlineOffset: 3 }} />)}</div></div>
...
function DetailMetric({ label, value }: { label: string; value: string }) { return <div className="min-w-0 bg-popover px-3 py-4"><p className="text-[10px] text-muted-foreground">{label}</p><p className="mt-1 truncate text-xs font-medium tabular-nums sm:text-sm">{value}</p></div>; }
