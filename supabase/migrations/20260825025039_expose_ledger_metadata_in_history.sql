    ...
    select movement.*,
      (
        select jsonb_build_object(
          ...
          'ledger_event_id', pair.ledger_event_id,
          'native_currency_code', pair.native_currency_code,
          'base_currency_code', pair.base_currency_code,
          'base_amount', pair.base_amount,
          'exchange_rate', pair.exchange_rate,
          'exchange_rate_date', pair.exchange_rate_date,
          'exchange_rate_source', pair.exchange_rate_source,
  ...
  select jsonb_build_object(
    'items', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'ledger_event_id', page.ledger_event_id,
        'native_currency_code', page.native_currency_code,
        'base_currency_code', page.base_currency_code,
        'base_amount', page.base_amount,
        'exchange_rate', page.exchange_rate,
        'exchange_rate_date', page.exchange_rate_date,
        'exchange_rate_source', page.exchange_rate_source,
...
comment on function public.get_transactions_page(integer, date, timestamptz, uuid, text, text, date, date, uuid, uuid) is
  'Cursor-paginated history with immutable ledger and currency snapshot metadata.';
