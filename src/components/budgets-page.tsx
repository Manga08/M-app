...
function BudgetEditor({ amount, onSave }: { amount: number; onSave: (amount: number) => void }) {
  ...
  return <div className="flex w-full items-center justify-between gap-2 sm:w-auto sm:justify-end sm:gap-1"><Button type="button" variant="outline" size="icon-sm" aria-label="Reducir presupuesto" onClick={() => setValue(Math.max(0, value - 50000))}><Minus className="size-3.5" /></Button><input aria-label="Monto presupuestado" className="h-11 min-w-0 flex-1 rounded-xl border bg-transparent px-3 text-right text-base tabular-nums outline-none focus:ring-2 focus:ring-ring sm:h-8 sm:w-24 sm:flex-none sm:rounded-lg sm:px-2 sm:text-sm" inputMode="numeric" value={value} onChange={(event) => setValue(Number(event.target.value.replace(/\D/g, "")))} /><Button type="button" variant="outline" size="icon-sm" aria-label="Aumentar presupuesto" onClick={() => setValue(value + 50000)}><Plus className="size-3.5" /></Button>{value !== amount ? <Button type="button" size="icon-sm" aria-label="Guardar presupuesto" onClick={() => onSave(value)}><Check className="size-3.5" /></Button> : null}</div>;
}
