-- Historial global: conserva la paginación por cursor y aplica todos los filtros
-- antes del límite. SECURITY INVOKER mantiene activas las políticas RLS.
drop function if exists public.get_transactions_page(integer, date, timestamptz, uuid, text, text, date, date);
drop function if exists public.get_transactions_page(integer, date, timestamptz, uuid, text, text, date, date, uuid, uuid);
  ...
  with candidate_rows as (
    ...
    where movement.user_id = caller_id
      ...
      and (
        ...
        or movement.account_id = p_account_id
        or (
          movement.kind = 'transfer_out'
          and exists (
            select 1
            from public.transactions destination
