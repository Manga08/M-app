...
async function choosePeriod(page: Page, label: "6 meses" | "1 año") {
  const quickOption = page.getByRole("button", { name: label, exact: true });
  if (await quickOption.isVisible()) {
    await quickOption.click();
    return;
  }

  await page.getByRole("button", { name: /^(Este mes|6 meses|1 año|2 años)$/ }).click();
...
test("el Excel usa los filtros y se descarga como xlsx", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Flujo de caja" })).toBeVisible({ timeout: 15_000 });
  const button = page.getByRole("button", { name: /Descargar reporte en Excel|Descargar Excel/ }).last();
