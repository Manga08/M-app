...
export function ReportsPage() {
  ...
  return <>
    ...
    {!connectionRequired && !online ? <p className="mb-6 rounded-xl bg-warning/8 px-4 py-3 text-xs text-warning">Sin conexión: estás usando el historial local completo de este modo.</p> : !connectionRequired && profile?.id !== "demo" && report?.source === "local" ? <p className="mb-6 rounded-xl bg-warning/8 px-4 py-3 text-xs text-warning">Mostrando el reporte completo con tus cambios financieros pendientes de sincronización.</p> : null}
...
export function ReportConnectionRequired() {
  return (
    <section className="grid min-h-80 place-items-center rounded-2xl bg-secondary/25 py-12" role="status" aria-live="polite" data-report-connection-required>
