...
export function DashboardPage() {
  ...
  const availableStatus = available > 0 ? `${availablePercent}% del ingreso disponible` : available === 0 ? "Sin margen disponible" : `${money.format(Math.abs(available))} por cubrir`;
  ...
  return <div className="min-w-0" data-dashboard>
    <section className="min-w-0 pb-8 md:border-b md:border-border/75 lg:pb-10" data-dashboard-hero>
      ...
      <div className="mt-8 grid min-w-0 grid-cols-2 gap-y-6 sm:mt-10 md:grid-cols-[minmax(0,1.65fr)_minmax(0,.675fr)_minmax(0,.675fr)] md:gap-0">
        <div className="col-span-2 min-w-0 [container-type:inline-size] md:col-span-1 md:pr-10 lg:pr-12"><p className="text-xs text-muted-foreground">Disponible este mes</p><p className={cn("mt-1 whitespace-nowrap text-[clamp(2.15rem,13cqi,4.25rem)] font-medium leading-none tracking-[-0.06em] tabular-nums", financialToneClass[availableSignal])} data-dashboard-balance>{money.format(available)}</p><div className={cn("mt-6 grid min-w-0 grid-cols-[minmax(7rem,1fr)_auto] items-center gap-4 text-xs", financialToneClass[availableSignal])}><span className={cn("h-2 w-full overflow-hidden rounded-full", available > 0 ? "bg-positive/15" : "bg-destructive/15")} role="progressbar" aria-label="Porcentaje del ingreso disponible" aria-valuemin={0} aria-valuemax={100} aria-valuenow={boundedAvailablePercent} aria-valuetext={availableStatus}><span className={cn("block h-full rounded-full", available > 0 ? "bg-positive" : "bg-destructive")} style={{ width: `${boundedAvailablePercent}%` }} /></span><span className="text-right tabular-nums">{availableStatus}</span></div></div>
        <Metric label="Ingresos" value={money.format(totals.income)} note={totals.income > 0 ? "Clasificados por tipo de ingreso" : "Sin ingresos registrados"} tone={availableTone(totals.income)} />
        <Metric label="Salidas" value={money.format(totals.expense)} note={totals.income > 0 ? `${Math.round((totals.expense / totals.income) * 100)}% de tus ingresos` : totals.expense > 0 ? "Salidas sin ingresos registrados" : "Sin salidas registradas"} tone={expenseTone(totals.expense)} mobileDivider />
      </div>
    </section>
    ...
    <section className="grid min-w-0 gap-9 py-8 lg:py-10 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.55fr)] xl:gap-12">
      ...
      <div className="min-w-0 xl:border-l xl:pl-9" data-dashboard-pulse>
        ...
        <div className="space-y-5"><Insight value={`${availablePercent}%`} label={available > 0 ? "del ingreso sigue disponible" : available === 0 ? "sin margen disponible" : "del ingreso, con déficit"} tone={availableSignal} /><Insight value={money.format(totals.expense / Math.max(currentDay, 1))} label="promedio diario de salida" tone={expenseTone(totals.expense)} /><Insight value={`${groups.filter((group) => group.percent >= 85).length} alertas`} label="categorías cerca del límite" tone={groups.some((group) => group.percent >= 85) ? "warning" : "positive"} /></div>
      </div>
    </section>
...
function Metric({ label, value, note, tone, mobileDivider = false }: { label: string; value: string; note: string; tone: FinancialTone; mobileDivider?: boolean }) { return <div className={cn("min-w-0 border-border md:border-l md:px-7", mobileDivider && "border-l pl-4 min-[360px]:pl-5 md:pl-7")}><p className="text-xs text-muted-foreground">{label}</p><p className={cn("mt-2 truncate text-[clamp(1.05rem,5.6vw,1.5rem)] font-medium tracking-[-0.04em] tabular-nums", financialToneClass[tone])}>{value}</p><p className="mt-1 text-pretty text-[11px] leading-4 text-muted-foreground min-[360px]:text-xs md:min-h-8">{note}</p></div>; }
...
function CategoryRow({ name, color, budget, spent, percent, icon, money }: ReturnType<typeof groupBudgetSummary>[number] & { icon: string; money: Intl.NumberFormat }) {
  const tone = budgetUsageTone(spent, budget);
  const status = tone === "destructive" ? "excedido" : tone === "warning" ? "cerca del límite" : tone === "positive" ? "en orden" : "sin presupuesto";
  const indicatorColor = tone === "destructive" ? "var(--destructive)" : tone === "warning" ? "var(--warning)" : color;
  return <Link href="/presupuestos" className="group grid min-h-[70px] w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-3 py-3 text-left transition-colors hover:bg-foreground/[0.025] active:bg-secondary/50 sm:grid-cols-[170px_minmax(120px,1fr)_130px_auto] sm:gap-4 sm:border-b"><span className="flex min-w-0 items-center gap-3 text-sm font-medium"><span className="grid size-9 shrink-0 place-items-center rounded-xl" style={{ color, backgroundColor: `${color}16` }}><FinanceIcon name={icon} className="size-[17px]" /></span><span className="truncate">{name}</span></span><span className="hidden h-1.5 overflow-hidden rounded-full bg-muted sm:block" role="progressbar" aria-label={`Presupuesto usado en ${name}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.max(0, Math.min(percent, 100))} aria-valuetext={`${percent}%, ${status}`}><span className="block h-full rounded-full" style={{ width: `${Math.max(0, Math.min(percent, 100))}%`, backgroundColor: indicatorColor }} /></span><span className="text-right"><span className={cn("block text-sm font-medium tabular-nums", tone !== "neutral" && financialToneClass[tone])}>{money.format(spent)}</span><span className="text-[11px] text-muted-foreground">de {money.format(budget)}</span></span><span className={cn("hidden w-12 text-right text-xs tabular-nums sm:block", financialToneClass[tone])}>{percent}%<span className="sr-only">, {status}</span></span></Link>;
}

function Insight({ value, label, tone = "neutral" }: { value: string; label: string; tone?: FinancialTone }) { return <div className="flex items-baseline gap-3 sm:border-b sm:pb-4"><span className={cn("text-xl font-medium tracking-tight tabular-nums", financialToneClass[tone])}>{value}</span><span className="text-sm text-muted-foreground">{label}</span></div>; }
...
function TransactionRow({ transaction, category, icon, money }: { transaction: Transaction; category?: string; icon: string; money: Intl.NumberFormat }) {
  ...
  return <Link href="/movimientos" className="group grid min-h-[66px] w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 py-2 text-left transition-colors hover:bg-foreground/[0.025] active:bg-secondary/50 sm:grid-cols-[auto_minmax(0,1.4fr)_minmax(120px,0.8fr)_auto_auto] sm:gap-4 sm:border-b"><span className={cn("grid size-9 place-items-center rounded-full", income ? "bg-positive/12 text-positive" : "bg-destructive/12 text-destructive")}><FinanceIcon name={icon} className="size-[17px]" /></span><span className="min-w-0"><span className="block truncate text-sm font-medium">{transaction.merchant || transaction.description}</span><span className="block truncate text-xs text-muted-foreground sm:hidden">{category || "Transferencia"}</span></span><span className="hidden text-xs text-muted-foreground sm:block">{category || "Transferencia"}</span><span className="hidden text-xs text-muted-foreground sm:block">{new Intl.DateTimeFormat("es-CO", { day: "2-digit", month: "short", timeZone: "UTC" }).format(new Date(`${transaction.occurredOn}T00:00:00Z`))}</span><span className={cn("text-right text-sm font-medium tabular-nums", income ? "text-positive" : "text-destructive")}>{income ? "+" : "-"}{money.format(transaction.amount)}</span></Link>;
}
