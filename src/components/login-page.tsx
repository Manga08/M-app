...
export function LoginPage() {
  ...
  return <main className="grid min-h-screen bg-background lg:grid-cols-[1.1fr_.9fr]">
    ...
    <section className="flex min-h-screen items-center justify-center p-6"><div className="w-full max-w-md"><div className="mb-12 flex items-center gap-2 lg:hidden"><BrandMark /><span className="text-lg font-semibold">Moneva</span></div><span className="grid size-12 place-items-center rounded-2xl bg-primary/12 text-primary"><Sparkles className="size-5" /></span><h2 className="mt-7 text-4xl font-medium tracking-[-.055em]">Bienvenido a tu calma financiera.</h2><p className="mt-4 text-sm leading-6 text-muted-foreground">Una cuenta privada, una sola fuente de verdad y todo lo necesario para entender tu mes.</p><Button onClick={signIn} disabled={!configured} className="mt-9 h-12 w-full rounded-full text-[15px]"><GoogleMark />Continuar con Google<ArrowRight className="ml-auto size-4" /></Button>{configured ? <p className="mt-4 flex items-center justify-center gap-2 text-xs text-muted-foreground"><LockKeyhole className="size-3.5" />Solo el correo autorizado puede entrar.</p> : <><Button asChild variant="outline" className="mt-3 h-12 w-full rounded-full"><Link href="/">Explorar modo demo</Link></Button><p className="mt-4 text-center text-xs leading-5 text-muted-foreground">Supabase aún no está conectado en este entorno. El modo demo guarda cambios en este dispositivo.</p></>}</div></section>
  </main>;
}
