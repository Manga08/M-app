...
export function ReportsPage() {
  ...
  const { profile, accounts, categories, online, pendingCount, syncing, getDetailedFinanceReport, exportReportTransactions, syncNow } = finance;
  ...
  return <>
    ...
    <section className="sticky top-[calc(68px+env(safe-area-inset-top))] z-20 -mx-4 border-y bg-background/96 px-4 py-3 backdrop-blur-md sm:static sm:mx-0 sm:rounded-[1.25rem] sm:border sm:bg-secondary/18 sm:p-2 sm:backdrop-blur-none" aria-label="Periodo y filtros del reporte">
      <div className="flex min-w-0 items-center gap-2 sm:flex-wrap">
        <div className="hidden shrink-0 rounded-full bg-secondary/75 p-1 sm:flex" role="group" aria-label="Periodo rápido">{PERIODS.map((item) => <button key={item.value} type="button" aria-pressed={query.preset === item.value} onClick={() => applyPreset(item.value)} className={cn("min-h-9 rounded-full px-3 text-xs font-medium text-muted-foreground transition-[background-color,color,box-shadow] duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", query.preset === item.value && "bg-background text-foreground shadow-sm")}>{item.label}</button>)}</div>
        <Button variant={query.preset === "custom" || query.preset === "months" ? "secondary" : "ghost"} className="h-11 min-w-0 flex-1 justify-start rounded-full px-4 sm:h-9 sm:flex-none" onClick={() => setFilterOpen(true)} title={mobilePeriodLabel}><CalendarRange className="size-4 shrink-0" /><span className="truncate sm:hidden">{mobilePeriodLabel}</span><span className="hidden sm:inline">{query.preset === "custom" || query.preset === "months" ? reportPeriodLabel(query) : "Otro periodo"}</span></Button>
        <div className="hidden h-6 w-px bg-border sm:block" />
        <Sheet open={filterOpen} onOpenChange={setFilterOpen}><SheetTrigger asChild><Button variant="outline" className="h-11 shrink-0 rounded-full px-4 sm:h-9"><SlidersHorizontal className="size-4" />Filtros{filterCount ? <span className="grid size-5 place-items-center rounded-full bg-primary text-[10px] text-primary-foreground">{filterCount}</span> : null}</Button></SheetTrigger><SheetContent side="right" className="w-full gap-0 p-0 sm:max-w-md"><ReportFilters query={query} onApply={(next) => { applyQuery(next); setFilterOpen(false); }} /></SheetContent></Sheet>
...
function ReportFilters({ query, onApply }: { query: ReportQuery; onApply: (query: ReportQuery) => void }) {
  ...
  return <>
    <SheetHeader className="border-b px-5 pb-4 pt-5"><SheetTitle>Configurar reporte</SheetTitle><SheetDescription>El periodo y estos filtros controlan toda la página y el Excel.</SheetDescription></SheetHeader>
    <div className="space-y-7 px-5 py-5">
      ...
      {draft.preset === "custom" ? <div className="grid grid-cols-2 gap-3"><DateField label="Desde" value={draft.startDate} onChange={(startDate) => setDraft((current) => ({ ...current, startDate }))} /><DateField label="Hasta" value={draft.endDate} onChange={(endDate) => setDraft((current) => ({ ...current, endDate }))} /></div> : null}
      {draft.preset === "months" ? <div className="space-y-3"><Label htmlFor="report-month">Meses específicos</Label><div className="flex gap-2"><Input id="report-month" type="month" value={monthInput} onChange={(event) => setMonthInput(event.target.value)} /><Button type="button" variant="outline" onClick={() => { if (monthInput) { setDraft((current) => ({ ...current, selectedMonths: [...new Set([...current.selectedMonths, monthInput])] })); setMonthInput(""); } }}>Agregar</Button></div><div className="flex flex-wrap gap-2">{draft.selectedMonths.map((month) => <button type="button" key={month} className="flex min-h-9 items-center gap-1 rounded-full bg-secondary px-3 text-xs" onClick={() => setDraft((current) => ({ ...current, selectedMonths: current.selectedMonths.filter((item) => item !== month) }))}>{formatMonth(month)}<X className="size-3" /></button>)}</div></div> : null}
      <div className="grid grid-cols-2 gap-3"><div className="space-y-2"><Label>Tipo</Label><Select value={draft.kind} onValueChange={(kind) => setDraft((current) => ({ ...current, kind: kind as ReportKindFilter }))}><SelectTrigger className="h-12 w-full"><SelectValue /></SelectTrigger><SelectContent position="popper"><SelectItem value="all">Todos</SelectItem><SelectItem value="expense">Gastos</SelectItem><SelectItem value="income">Ingresos</SelectItem><SelectItem value="transfer">Transferencias</SelectItem></SelectContent></Select></div><div className="space-y-2"><Label>Comparar con</Label><Select value={draft.comparison} onValueChange={(comparison) => setDraft((current) => ({ ...current, comparison: comparison as ReportQuery["comparison"] }))}><SelectTrigger className="h-12 w-full"><SelectValue /></SelectTrigger><SelectContent position="popper"><SelectItem value="previous">Periodo anterior</SelectItem><SelectItem value="year">Año anterior</SelectItem><SelectItem value="none">Sin comparación</SelectItem></SelectContent></Select></div></div>
...
function FilterChecks({ title, items, selected, onToggle }: { title: string; items: Array<{ id: string; label: string; color: string }>; selected: string[]; onToggle: (id: string) => void }) { if (!items.length) return null; return <fieldset><legend className="mb-3 text-sm font-medium">{title}</legend><div className="grid gap-1 sm:grid-cols-2">{items.map((item) => <label key={item.id} className="flex min-h-11 cursor-pointer items-center gap-3 rounded-xl px-2 text-sm hover:bg-secondary/55"><Checkbox checked={selected.includes(item.id)} onCheckedChange={() => onToggle(item.id)} /><i className="size-2.5 rounded-full" style={{ backgroundColor: item.color }} /><span className="min-w-0 truncate">{item.label}</span></label>)}</div></fieldset>; }
function DateField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { const id = `report-${label.toLowerCase()}`; return <div className="space-y-2"><Label htmlFor={id}>{label}</Label><Input id={id} type="date" value={value} onChange={(event) => onChange(event.target.value)} /></div>; }
...
function AccountReport({ report, money }: { report: DetailedFinanceReport; money: Intl.NumberFormat }) { return <section className="border-b py-9"><SectionHeading eyebrow="Patrimonio" title="Flujo por cuenta" description="Saldo al inicio, movimiento neto y saldo al cierre del periodo." /><div className="mt-5 grid gap-x-8 md:grid-cols-2">{report.accounts.map((account) => <div key={account.id} className="grid min-h-20 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 border-b py-4"><span className="grid size-11 place-items-center rounded-xl" style={{ backgroundColor: `${account.color}20`, color: account.color }}><WalletCards className="size-5" /></span><div className="min-w-0"><p className="truncate text-sm font-medium">{account.name}</p><p className={cn("mt-1 text-xs tabular-nums", account.netFlow === 0 ? "text-muted-foreground" : financialToneClass[availableTone(account.netFlow)])}>{account.netFlow > 0 ? "+" : ""}{money.format(account.netFlow)} en el periodo</p></div><div className="text-right"><p className="text-sm font-medium tabular-nums">{money.format(account.closingBalance)}</p><p className="text-xs text-muted-foreground">antes {money.format(account.openingBalance)}</p></div></div>)}</div></section>; }
...
function ExactSeriesData({ report, money }: { report: DetailedFinanceReport; money: Intl.NumberFormat }) { return <details className="mt-5 min-w-0 w-full max-w-full overflow-hidden border-t pt-2"><summary className="flex min-h-11 cursor-pointer items-center text-sm font-medium text-primary">Ver datos exactos del gráfico</summary><div className="mt-2 divide-y sm:hidden">{report.series.map((item) => <div key={item.period} className="py-3"><p className="text-sm font-medium">{formatDateBucket(item.period, report.granularity)}</p><dl className="mt-2 grid grid-cols-3 gap-2 text-xs"><div><dt className="text-muted-foreground">Ingresos</dt><dd className="mt-1 truncate tabular-nums">{money.format(item.income)}</dd></div><div><dt className="text-muted-foreground">Gastos</dt><dd className="mt-1 truncate tabular-nums">{money.format(item.expense)}</dd></div><div className="text-right"><dt className="text-muted-foreground">Balance</dt><dd className="mt-1 truncate tabular-nums">{money.format(item.balance)}</dd></div></dl></div>)}</div><div className="mt-2 hidden max-w-full overflow-x-auto overscroll-x-contain sm:block"><table className="w-full min-w-[520px] border-collapse text-sm"><caption className="sr-only">Ingresos, gastos y balance exactos de cada periodo</caption><thead><tr className="border-b text-left text-xs text-muted-foreground"><th className="py-2 font-normal">Periodo</th><th className="px-3 py-2 text-right font-normal">Ingresos</th><th className="px-3 py-2 text-right font-normal">Gastos</th><th className="py-2 text-right font-normal">Balance</th></tr></thead><tbody>{report.series.map((item) => <tr key={item.period} className="border-b last:border-0"><th scope="row" className="py-2 text-left font-medium">{formatDateBucket(item.period, report.granularity)}</th><td className="px-3 py-2 text-right tabular-nums">{money.format(item.income)}</td><td className="px-3 py-2 text-right tabular-nums">{money.format(item.expense)}</td><td className="py-2 text-right tabular-nums">{money.format(item.balance)}</td></tr>)}</tbody></table></div></details>; }
...
function ReportLoading() { return <div className="grid gap-6 py-8" aria-label="Calculando reporte" aria-busy="true"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{Array.from({ length: 4 }, (_, index) => <Skeleton key={index} className="h-28 rounded-2xl" />)}</div><Skeleton className="h-[390px] rounded-2xl" /><div className="grid gap-6 lg:grid-cols-2"><Skeleton className="h-72 rounded-2xl" /><Skeleton className="h-72 rounded-2xl" /></div></div>; }
