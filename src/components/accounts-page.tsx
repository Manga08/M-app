...
import { FormControl, FormControlAdornment, FormControlInput, SelectControl } from "@/components/ui/form-control";
...
export function AccountsPage() {
  const { profile, accounts, categories, transactions, snapshot, mutate } = useFinance();
  ...
  return <>
    ...
    <div className="grid min-w-0 gap-10 pt-8 xl:grid-cols-[minmax(0,1.08fr)_minmax(360px,.92fr)] xl:gap-14 xl:pt-10">
      <section className="min-w-0"><div className="mb-5"><h2 className="text-xl font-medium tracking-tight">Todas las cuentas</h2><p className="mt-1 text-sm text-muted-foreground">Saldo calculado con tus movimientos.</p></div><div role="list">{visibleBalances.map(({ account, balance }) => <div key={account.id} role="listitem" className="grid min-h-[82px] w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 border-b py-3 sm:gap-4"><span className="grid size-11 place-items-center rounded-2xl" style={{ backgroundColor: `${account.color}1f`, color: account.color }}><FinanceIcon name={account.icon || typeIcon[account.type]} className="size-7" /></span><span className="min-w-0"><span className="block truncate text-sm font-medium">{account.name}</span><span className="block truncate text-xs text-muted-foreground">{typeLabel[account.type]}</span></span><span className={cn("text-right text-sm font-medium tabular-nums sm:text-base", balance < 0 && "text-destructive")}>{money.format(balance)}</span></div>)}</div><PaginationControls page={page} pageCount={pageCount} onPageChange={setPage} total={balances.length} label="cuentas" /></section>
      <section id="tipos-de-ingreso" className="min-w-0 scroll-mt-24 border-t pt-8 xl:border-t-0 xl:pt-0">
