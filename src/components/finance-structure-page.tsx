...
function CategoryRow({ category, canMoveUp, canMoveDown, onMove, onEdit, onArchive }: { category: Category; canMoveUp: boolean; canMoveDown: boolean; onMove: (direction: -1 | 1) => void; onEdit: () => void; onArchive: () => Promise<void> }) {
  return <div className="grid min-h-16 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 border-b py-2">
    <span className="grid size-10 place-items-center rounded-xl" style={{ color: category.color, backgroundColor: `${category.color}16` }}><FinanceIcon name={category.icon} className="size-[18px]" /></span><div className="min-w-0"><p className="truncate text-sm font-medium">{category.name}</p>{category.isDefault ? <p className="truncate text-[11px] text-muted-foreground">Subcategoría inicial · totalmente editable</p> : null}</div>
...
function GroupDialog({ open, group, nextOrder, onOpenChange, onSave }: { open: boolean; group?: GroupAllocation; nextOrder: number; onOpenChange: (open: boolean) => void; onSave: (group: FinanceGroupInput) => Promise<void> }) {
  ...
  const [color, setColor] = useState(group?.color ?? FINANCE_IDENTITY_COLORS[nextOrder % FINANCE_IDENTITY_COLORS.length]);
  ...
  return <Dialog open={open} onOpenChange={(next) => !saving && onOpenChange(next)}><FormDialogContent variant="compact" showCloseButton={!saving}><form onSubmit={submit} className="flex min-h-0 flex-1 flex-col"><FormDialogBody><DialogHeader className="pr-8"><p className="text-xs uppercase tracking-[.14em] text-primary">Estructura del plan</p><DialogTitle>{group ? "Editar categoría principal" : "Crear categoría principal"}</DialogTitle><DialogDescription>El nombre, color e icono se aplicarán a todas las vistas. Después podrás decidir si participa del porcentaje.</DialogDescription></DialogHeader><FinanceIdentityField id="group-name" value={name} onValueChange={setName} icon={icon} onIconChange={setIcon} color={color} onColorChange={setColor} maxLength={60} required disabled={saving} autoFocus placeholder="Ej. Educación" colorLabel="Color de la categoría principal" /></FormDialogBody><FormDialogActions><Button type="submit" className="h-12 w-full rounded-2xl sm:w-auto" disabled={!name.trim() || saving}>{saving ? <LoaderCircle className="size-4 animate-spin" /> : null}{saving ? "Guardando…" : group ? "Guardar cambios" : "Crear categoría"}</Button><Button type="button" variant="outline" className="mt-2 h-12 w-full rounded-2xl sm:mt-0 sm:w-auto" onClick={() => onOpenChange(false)} disabled={saving}>Cancelar</Button></FormDialogActions></form></FormDialogContent></Dialog>;
}
...
function CategoryDialog({ open, category, initialGroup, groups, onOpenChange, onSave }: { open: boolean; category?: Category; initialGroup?: string; groups: GroupAllocation[]; onOpenChange: (open: boolean) => void; onSave: (category: { id: string; name: string; group: string; color: string; icon: string }) => Promise<void> }) {
  ...
  const initialColor = category?.color ?? groups.find((item) => item.group === (initialGroup ?? groups[0]?.group))?.color ?? FINANCE_IDENTITY_COLORS[0];
  ...
  async function submit(event: React.FormEvent) {
    ...
    setSaving(true);
    try { await onSave({ id: category?.id ?? crypto.randomUUID(), name: name.trim(), group: group.group, color, icon }); }
    catch (error) { announceMutationError(error, "No pudimos guardar la subcategoría."); }
    finally { setSaving(false); }
  }
  return <Dialog open={open} onOpenChange={(next) => !saving && onOpenChange(next)}><FormDialogContent variant="compact" showCloseButton={!saving}><form onSubmit={submit} className="flex min-h-0 flex-1 flex-col"><FormDialogBody><DialogHeader className="pr-8"><p className="text-xs uppercase tracking-[.14em] text-primary">Clasificación del gasto</p><DialogTitle>{category ? "Editar subcategoría" : "Nueva subcategoría"}</DialogTitle><DialogDescription>Úsala para clasificar gastos con precisión. Puedes moverla y personalizar su identidad cuando quieras.</DialogDescription></DialogHeader><FinanceIdentityField id="category-name" value={name} onValueChange={setName} icon={icon} onIconChange={setIcon} color={color} onColorChange={(value) => { setColor(value); setColorTouched(true); }} required disabled={saving} autoFocus placeholder="Ej. Cursos y libros" colorLabel="Color de la subcategoría" /><div className="mt-5"><Label htmlFor="category-group">Categoría principal</Label><SelectControl id="category-group" value={groupKey} onValueChange={(value) => { setGroupKey(value); if (!colorTouched) setColor(groups.find((item) => item.group === value)?.color ?? FINANCE_IDENTITY_COLORS[0]); }} containerClassName="mt-2" disabled={saving}>{groups.map((group) => <option key={group.group} value={group.group}>{group.name}</option>)}</SelectControl><p className="mt-1.5 text-[11px] leading-4 text-muted-foreground">Moverla conserva su historial, icono y color propio.</p></div></FormDialogBody><FormDialogActions><Button type="submit" className="h-12 w-full rounded-2xl sm:w-auto" disabled={!name.trim() || !groupKey || saving}>{saving ? <LoaderCircle className="size-4 animate-spin" /> : null}{saving ? "Guardando…" : category ? "Guardar cambios" : "Crear subcategoría"}</Button><Button type="button" variant="outline" className="mt-2 h-12 w-full rounded-2xl sm:mt-0 sm:w-auto" onClick={() => onOpenChange(false)} disabled={saving}>Cancelar</Button></FormDialogActions></form></FormDialogContent></Dialog>;
}
